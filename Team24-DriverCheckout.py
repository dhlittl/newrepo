...
def lambda_handler(event, context):
    try:
        ...
        if method == 'POST':
            ...
            try:
                # Print debug info
                print(f"Processing order for driver_id={driver_id}, total_points={total_points}")
                ...
                # 2. Check if driver has enough points in the Driver_To_SponsorOrg table
                cursor.execute(
                    """
                ...
                if current_balance < total_points:
                    print(f"Insufficient points balance: {current_balance} < {total_points}")
                ...
                # 4. Create invoice/order
                order_id = str(uuid.uuid4())
                order_date = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                
                print(f"Generated order_id={order_id}, order_date={order_date}")
                
                # 5. Deduct points from driver in Driver_To_SponsorOrg table
                print(f"Deducting {total_points} points from Driver_ID={actual_driver_id} for Sponsor_Org_ID={sponsor_org_id}")
                cursor.execute(
                    """
                    ...
                    WHERE Driver_ID = %s AND Sponsor_Org_ID = %s
                    """,
                    (total_points, actual_driver_id, sponsor_org_id)
                )
                print(f"Points deduction result: {cursor.rowcount} rows affected")
                
                # 6. Record point change in audit log
                print(f"Recording point change in audit log")
                cursor.execute(
                    """
                    INSERT INTO Point_Changes (
                        Driver_ID, Sponsor_Org_ID, Sponsor_User_ID, Num_Points, Point_Change_Type, Reason, Change_Date
                    ) VALUES (
                        %s, %s, NULL, %s, 'SUB', 'Product purchase', %s
                    )
                    """,
                    (actual_driver_id, sponsor_org_id, -total_points, order_date)
                )
                print(f"Audit log result: {cursor.rowcount} rows affected")
                
                # 7. Create purchase record in Purchase table
                print(f"Creating purchase record with order_id={order_id}")
                cursor.execute(
                    """
                ...
                # 8. Process items and create purchase_item records
                ...
                for item in items:
                    ...
                    print(f"Updating product quantity: Product_ID={product_id}, decreasing by {quantity}")
                    cursor.execute(
                        "UPDATE Product SET Quantity = Quantity - %s WHERE Product_ID = %s",
                        (quantity, product_id)
                    )
                    print(f"Product quantity update result: {cursor.rowcount} rows affected")
                ...
                # 9. Update driver's purchase count
                print(f"Updating driver purchase count for Driver_ID={actual_driver_id}")
                cursor.execute(
                    "UPDATE Driver SET Num_Purchases = Num_Purchases + 1 WHERE Driver_ID = %s",
                    (actual_driver_id,)
                )
                print(f"Purchase count update result: {cursor.rowcount} rows affected")
                ...
                result = {
                    ...
                    'newBalance': current_balance - total_points
                }
        ...
        # Get order history for a driver (GET method)
            ...
            # Query purchases for the driver
            cursor.execute(
                """
                SELECT 
                ...
                WHERE 
                    p.Driver_ID = %s
                GROUP BY
                    p.Purchase_ID, p.Purchase_Date, p.Total_Points, p.Status, p.Sponsor_Org_ID, so.Sponsor_Org_Name
                ORDER BY 
