...
def lambda_handler(event, context):
    try:
        ...
        if method == 'PUT':
            ...
            cursor.execute("SELECT * FROM Sponsor_Preferences WHERE user_id = %s", (user_id,))
            record = cursor.fetchone()

            if record:
                print(f"Record found for User_ID: {user_id}, updating widget order...")
                cursor.callproc('UpdateSponsorWidgetOrder', (user_id, json.dumps(widget_order)))
            else:
                print(f"No record found for User_ID: {user_id}, inserting widget order...")
                cursor.execute("INSERT INTO Sponsor_Preferences (user_id, widget_order) VALUES (%s, %s)", (user_id, json.dumps(widget_order)))
            ...
            cursor.callproc('GetSponsorWidgetOrder', (User_ID,))
            widget_order = None
