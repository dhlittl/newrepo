...
def lambda_handler(event, context):
    try:
        ...
        if method == 'PUT':
            ...
            cursor.execute("SELECT * FROM Driver_Preferences WHERE user_id = %s", (user_id,))
            record = cursor.fetchone()

            if record:
                print(f"Record found for User_ID: {user_id}, updating widget order...")
                cursor.callproc('UpdateDriverWidgetOrder', (user_id, json.dumps(widget_order)))
            else:
                print(f"No record found for User_ID: {user_id}, inserting widget order...")
                cursor.execute("INSERT INTO Driver_Preferences (user_id, widget_order) VALUES (%s, %s)", (user_id, json.dumps(widget_order)))
            ...
            cursor.callproc('GetDriverWidgetOrder', (User_ID,))
            widget_order = None
